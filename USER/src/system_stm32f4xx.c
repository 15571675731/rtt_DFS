...
void SystemInit(void)
{
  ...
  /* Set HSION bit */
  RCC->CR |= (uint32_t)0x00000001; //使能hsi，其他的全部关闭

  /* Reset CFGR register */
  RCC->CFGR = 0x00000000; //选择hsi为系统时钟,
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)//HSE稳定后配置HSE
  {
    ...
    /* Configure the main PLL */ /* pll_m=8  pll_n=336 pll_p=2 */ /* PLLCLK = hse/pll_m*pll_n/pll_p = 8/8*336/2 = 168 */
    RCC->PLLCFGR = PLL_M | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) |
                   (RCC_PLLCFGR_PLLSRC_HSE) | (PLL_Q << 24); //pll_clk=168

    /* Enable the main PLL */
    RCC->CR |= RCC_CR_PLLON;/* 手册上说要先配置再使能，因为PLL一旦使能就不能修改配置了 */
    ...
    /* Select the main PLL as system clock source */ /* 选择主PLL作为时钟源 */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_SW));/*切换为HSI*/
    RCC->CFGR |= RCC_CFGR_SW_PLL;//切换为HSE
